...
# #EatHab
# save(EatHab,mcdonald,alcohol,SweetBev,file = "EatHab.RData")
# 
# Major <- na.omit(Age, education, Major[-1,])
# #Major
# save(Age, education, Major,file = "Major.RData")
# 
# #GDP
...
From Parllel Coordinate Plot, we can see the trend more clear.
...
df_alcohol
ggplot(df_alcohol)+geom_bar(aes(x=reorder(State,Rank),y=Num.Drink.per.Capita,fill=-Rank),stat = "identity")+coord_flip()
```
...
ggplot(merge(df_percent %>%select(1,9),mcdonald%>%select(1,3),by="State"))+geom_bar(aes(x=reorder(State,Rank),y=McDonald.s.per.1M),stat="identity")+coord_flip()
```
