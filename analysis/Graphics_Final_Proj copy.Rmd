...
For those States with low population like Virginia Island, Hawaii, we just simply eliminate those data from the dataframe for reducing the effects of potential outliers. However, for data as Sweet Beverage, we were only able to find the data for 24 states, which caused a lot of missing values compared with other datasets have 50+ states. Hence, the only we of dealing with this kind of situation was to separate it from the whole big dataset in case it affect other analysis.
...
Six potential minor risk factors above approximately show existing positively correlation with diagnosed Diabetes. Even though High Cholesterol and Smoking slightly increase as proportion of diagnosed diabetes increases, the rest factors including Hypertension, Inactivity, Obesity and poor health are all showing a growing trend, which tells us that as proportion of diagnosed diabetes increases, the proportion of those risk factors increase as well. Therefore, these minor risk factors do have influences on causing diabetes to some degree in the long run.
...
#df_all
ggplot(df_all[df_all$RiskFactor!="Num",],aes(x=RiskFactor,y=Percent,fill=RiskFactor))+geom_dotplot(binaxis = "y",stackdir = "center",alpha=0.6,col="white",binwidth = 2)+ggtitle("Risk Factor Percentage of Diabetes")
...
Originally, we presumed that alcohol will definitely increase the chance of being diabetes. Here we use horizontal bar plot to explore the relation between diabetes and excessive alcohol assumption. We ranked the states in the order from largest diabetes population in the top to least diabetes population in the bottom. Surprisingly, it appears no clear pattern or even a trend which approximately shows that number of excessive drinks per capita increases as number of diagnosed diabetes decreases that opposing against our original belief. However, after reviewing the materils about diabetes, it was found that excessive drinking may indeed cause a decrease in blood sugar, but this does not mean that excessive drinking can help relieve diabetes, even though the amount and types of alcohol might relate to diabetes in some way which requires for further researching. 
...
ggplot(merge(df_percent %>%select(1,9)%>%filter(State != "Tennessee"&State!="Florida"),mcdonald%>%select(1,3),by="State"))+
...
Considering that diet habits specially fast foods could be a important factor for diabetes, we are able to draw a cleveland dot pot to make comparion of number of McDonald's location among all states. Even though there is not a clear line to show the relation between diabetes and number of McDonald's location, we are still able to observe a slightly increasng trend that number of McDonalds increases as the diabetes population of the state increases. Therefore we decide to draw another plot to see if this is the case.
...
mosaic(McDonald.s.per.1M~Rank, 
       gp = gpar(fill = c("#ffff99", "#2b83ba"),col = "white",alpha=1),spacing = spacing_equal(sp = unit(0.2, "lines")), labeling = labeling_border(rot_labels = c(0, 0, 0, 45)),mo_df)
...
We divided states into five ranges in terms of ranking in diabetes population, and at the same time, the number of McDonalds are classified in 3 categories (less than 44 McDonalds per 1 million people, 45-54 McDonalds per 1 million people, greater than 55 McDonalds per 1 million people). The top 20 diabetes states has much larger proportion on 55+ McDonalds comparing with the bottom 20 diabetes states. 
...
```{r}
ggplot(A)+geom_point(aes(x=reorder(State,Rank),y=Percent,col=Freq),alpha = 0.7, size = 4)+coord_flip()+ylab("Percent")+xlab("State(ordered by number of diabetes)")+ggtitle("State vs. Sweet Beverage Consumption")+theme_cleveland()
```

Does drinking sweet beverages increases the possibility of diabetes? The above cleveland dot plot giving the answer is Yes. When the states have larger amount of diabetes, the lower percentage people do not drink sweet beverage at all and higher percentage people drink more than 1 per day. In other words, percentage of more than 1 per day (green dots) increases as the state's number of diabetes increases; percentages of less than 1 per day and no sweet beverage decreases as the state's number of diabetes increases. We believe that sweet beverage consumption does cause diabetes. Let's see another plot to ensure this result.
...
g1+theme_economist()+theme(axis.text.x = element_text(angle = 90))+xlab("States")
...
This parallel coordinate plot also provided the evidence that indicating sweet drinks contribute to development of diabetes. The more diabetes states have (purple line), the larger percentage of more1perDay (green line) they have, and the less percentage of less1perDay (red line) and NoSweetBev (blue line) they have. Drinking sweet beverages seems to be an important factor to become diabetes.
...
An overall correlation between every two factors from per capita GDP, sweet beverages and fast food (McDonald) can be shown from above pairs plot. The higher the per capita GDP is, the less number the McDonald has. Moreover, the higher the per capita GDP is, the less likely people drink sweet beverages. In a sense, with higher per capita GDP condition, people tends to healthier eating habits (less fastfood and sweet beverages), which reduces the chance of diagnosed diabetes.
